...
def reshape_data(data, num_frames, num_features, modeltype):
    ...
    #   LSTM autoencoder
    if (modeltype == AUTOENCODER_MODEL_TYPE.LSTM or modeltype == AUTOENCODER_MODEL_TYPE.NONE):
        data = data.reshape(num_frames, SEQUENCE_LENGTH, num_features)
...
# loads all the data from a given session
def load_session_data(session):
    X_all = list()
    y_all = list()
    subjects = users_of_a_folder(session)
    num_subjects = len(subjects)
    SUBJECTS = subjects[0:num_subjects - 1]
    for subject in SUBJECTS:
        recordings = files_of_a_folder(session, subject)
        num_recordings = len(recordings)
        X = read_recording(ZJU_BASE_FOLDER+'/' + session + '/' + subject + '/' + recordings[0] + '/' + '3.txt', modeltype)
        for i in range(1, num_recordings):
            X_temp = read_recording(ZJU_BASE_FOLDER+'/' + session + '/' + subject + '/' + recordings[i] + '/' + '3.txt', modeltype)
            X = np.concatenate((X, X_temp), axis=0)
        y = np.full((X.shape[0], 1), subject)
